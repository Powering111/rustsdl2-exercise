...
impl<'a> Engine<'a> {
    pub fn new() -> Result<Self, Error> {
        let sdl_context = sdl2::init().map_err(|err| Error::InitFailure(format!("sdl2 initialization failed : {err}")))?;

        let video_subsystem = sdl_context.video().map_err(|err| Error::InitFailure(format!("video subsystem initialization failed : {err}")))?;
        let audio_subsystem = sdl_context.audio().map_err(|err| Error::InitFailure(format!("audio subsystem initialization failed : {err}")))?;
    ...
    pub fn main_loop(mut self){
        loop {
            ...
            for keycode in self.event_pump
                .keyboard_state()
                .pressed_scancodes()
                .filter_map(Keycode::from_scancode)
            {
                match keycode {
                    Keycode::W => curr_scene.set_position(curr_scene.get_position() + Vec2 { x: 0, y: 10 }),
                    Keycode::A => curr_scene.set_position(curr_scene.get_position() + Vec2 { x: -10, y: 0 }),
                    Keycode::S => curr_scene.set_position(curr_scene.get_position() + Vec2 { x: 0, y: -10 }),
                    Keycode::D => curr_scene.set_position(curr_scene.get_position() + Vec2 { x: 10, y: 0 }),
                    _ => (),
                }
            }
